...
const TransactionConfirmationDialog = ({ details, onConfirm, showConfirmButton = true }: { details: TransactionDetails, onConfirm?: () => void, showConfirmButton?: boolean }) => (
    <DialogContent>
        ...
        <div className="space-y-4 py-4 text-sm">
            <div className="flex justify-between items-center"><span className="text-muted-foreground">De:</span><span className="font-mono truncate max-w-[50%]">{details.sender}</span></div>
            <div className="flex justify-between items-center"><span className="text-muted-foreground">À:</span><span className="font-mono truncate max-w-[50%]">{details.receiver}</span></div>
            <div className="flex justify-between items-center font-semibold text-lg text-primary"><span>Montant:</span><span>{details.amount} {details.asset}</span></div>
            <div className="flex justify-between items-center"><span className="text-muted-foreground">Frais de réseau:</span><span>{details.fee} SAHEL</span></div>
            <div className="flex flex-col gap-1 pt-2 border-t"><span className="text-muted-foreground text-xs">ID de Transaction:</span><span className="font-mono text-xs break-all">{details.transactionId}</span></div>
        </div>
    ...
    return (
        <AppLayout>
            <div className="space-y-6">
                ...
                {transactionDetails && (
                    <>
                        <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}><TransactionConfirmationDialog details={transactionDetails} onConfirm={handleConfirmSend} /></Dialog>
                        <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}><TransactionConfirmationDialog details={transactionDetails} showConfirmButton={false} /></Dialog>
                    </>
                )}
